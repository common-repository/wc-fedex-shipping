...
class Plugin
{
	...
	{		
		...
		$validationErrors = WC()->session->get($this->id . '_validationErrors');
	...
	{
		if (!isset(WC()->session) || !isset(WC()->cart) || $this->settings['fetchRatesPageCondition'] != 'checkout') {
			return;
		}

		if (!apply_filters($this->id . '_isCheckout', false)) {
			WC()->session->set($this->id . '_' . __FUNCTION__, false);
			return;
		}
		...
		if (is_array($packages) && !empty($packages) && !WC()->session->get($this->id . '_' . __FUNCTION__)) {
			...
			WC()->session->set($this->id . '_' . __FUNCTION__, true);
		}
	}
