...
class ShippingMethod extends AbstractShippingMethod
{
	...
	{
		return $this->adapter->getPackageTypes($destination);
	}
		...
		if ($this->adapter->hasAddressValidationFeature()) {
			$this->form_fields += array(
				'validateAddress' => array(
					...
					'label' => __('Require shipping address to be validated before accepting an order', $this->id),
					'type' => 'checkbox',
					'default' => 'no'
				),
			);	
	...
	protected function getOriginFormFields()
	{
		...
		if (empty($this->instance_id)) {
			if (isset($this->settings['origin']['country'])) {
				$states = WC()->countries->get_states($this->settings['origin']['country']);
			}
		} else {
			if (isset($this->instance_settings['origin']['country'])) {
				$states = WC()->countries->get_states($this->instance_settings['origin']['country']);
			}
		}
	...
	public function calculate_shipping($package = array())
	{
		...
		if (empty($package) || empty($package['contents'])) {
			...
			$package['contents'] = isset(WC()->cart) ? WC()->cart->get_cart() : array();	
		...
		$packageTypes = $this->getPackageTypes($package['destination']);
	...
	protected function findShippingRates($package, $parcel)
	{
		...
		if (empty($rates)) {
			...
			WC()->session->set($this->id . '_validationErrors', $validationErrors);
	...
	protected function getCustomerAddress()
	{
		...
		foreach ($fieldsMap as $field => $toKey) {
			...
			if (empty($value) && is_object($customer) && method_exists($customer, "get_shipping_{$field}")) {
				$value = $customer->{"get_shipping_{$field}"}();
			}
			
			if (empty($value) && is_object($customer) && method_exists($customer, "get_billing_{$field}")) {
				$value = $customer->{"get_billing_{$field}"}();
			}
